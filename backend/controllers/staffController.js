...
exports.addStaff = async (req, res) => {
  try {
    ...
    if (!firstName || !lastName || !email || !department || !password) {
      return res
        .status(400)
        .json({ message: "First name, last name, email, department, and password are required" });
    }
    ...
    res.status(201).json({
      ...
      staff: {
        ...
        department: savedStaff.department || "N/A",
        position: savedStaff.position || "N/A",
        phone: savedStaff.phone || "",
        kpis: 0,
        completion: 0,
        avatarColor: generateColorFromEmail(savedStaff.email),
      },
    });
  } catch (error) {
